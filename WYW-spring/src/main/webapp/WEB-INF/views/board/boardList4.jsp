                ...
                <tr>
                    ...
                    <c:choose>
                        <c:when test="${QuestionDto.reg_date.time >= startOfToday}">
                            <td class="regdate" id="nav_DATE_Value"><fmt:formatDate value="${QuestionDto.reg_date}" pattern="HH:mm" type="time"/></td>
                        </c:when>
                        <c:otherwise>
                            <td class="regdate" id="nav_DATE_Value"><fmt:formatDate value="${QuestionDto.reg_date}" pattern="yyyy-MM-dd" type="date"/></td>
                        </c:otherwise>
                    </c:choose>
                    <td id="nav_VIEWS_Value">${QuestionDto.view_cnt}</td>
                </tr>
           </tbody>
           		</c:forEach>
        </table>
        ...
        <div class="main-page__mid">
            <div class="button">
                ...
                <c:if test="${totalCnt!=null && totalCnt!=0}">
                    <c:if test="${ph.showPrev}">
                        <a id="btn_left" href="<c:url value="/board4/list4${ph.sc.getQueryString(ph.beginPage-1)}"/>"></a>
                    </c:if>
                    <c:forEach var="i" begin="${ph.beginPage}" end="${ph.endPage}">
                        <a id="btn_mid" ${i==ph.sc.page? "paging-active" : ""}" href="<c:url value="/board4/list4${ph.sc.getQueryString(i)}"/>">${i}</a>
                    </c:forEach>
                    <c:if test="${ph.showNext}">
                        <a id="btn_right" href="<c:url value="/board4/list4${ph.sc.getQueryString(ph.endPage+1)}"/>"></a>
                    </c:if>
                </c:if>
            </div>
        </div>
         ...
         <div class="main-page__Bottom">
          <div class="Search_word">
            ...
            <form action="<c:url value="/board4/list4"/>" class="search-form" method="get">
            ...
            <button id="writeBtn" class="btn-write" onclick="location.href='<c:url value="/board4/write4"/>'">글쓰기</button>
           </div>
        </div>  
    </div>
</div>
